...
function handle_admin(string $path, string $method, array $cfg): never {
    ...
    if ($path === '/admin/settings' && $method === 'POST') {
        ...
        $newUrl   = trim($_POST['site_url'] ?? '');
        $newDesc  = trim($_POST['site_description'] ?? '');
        $newPass  = $_POST['new_password'] ?? '';
        $confirmPass = $_POST['confirm_password'] ?? '';

        if ($newTitle === '' || $newUrl === '') {
            $_SESSION['flash'] = 'Site title and URL are required.';
            header('Location: /admin');
            exit;
        }

        // Password: only update if both fields filled
        $hash = $cfg['admin_password'];
        ...
        $safeTitle = addslashes($newTitle);
        $safeUrl   = addslashes($newUrl);
        $safeDesc  = addslashes($newDesc);
...
function render_admin_dashboard(array $cfg): never {
    ...
    // ─── Settings section ──────────────────────────────────────────────────
    echo '<div class="section">';
    echo '<div class="section-header collapsed" onclick="toggleSection(this)">';
    echo '<h2>Settings</h2>';
    echo '<div class="meta"><span class="chevron">▼</span></div>';
    echo '</div>';

    echo '<div class="section-body hidden">';
    echo '<form method="POST" action="/admin/settings">';
    echo '<div class="edit-grid">';
    echo '<label>Site Title<input type="text" name="site_title" value="' . e($cfg['site_title'] ?? '') . '" required></label>';
    echo '<label>Site URL<input type="url" name="site_url" value="' . e($cfg['site_url'] ?? '') . '" required></label>';
    echo '<label class="full-width">Site Description<input type="text" name="site_description" value="' . e($cfg['site_description'] ?? '') . '"></label>';
    echo '<label>New Password<input type="password" name="new_password" placeholder="blank = no change"></label>';
    echo '<label>Confirm Password<input type="password" name="confirm_password" placeholder="blank = no change"></label>';
    echo '</div>';
    echo '<div class="edit-actions"><button type="submit">Save</button></div>';
    echo '</form>';
    echo '</div></div>';
